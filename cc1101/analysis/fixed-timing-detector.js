 ...
 *
 * @typedef {object} FixedTimingDetectionResult
 * @property {number} id
 * @property {string} ts
 ...
 *
 * @typedef {object} FixedTimingDetectorOptions
 * @property {number=} bus
 ...
 * @property {(result: FixedTimingDetectionResult) => void=} onDetection
 */

class CC1101FixedTimingDetector {
  /**
   * @param {FixedTimingDetectorOptions=} options
   */
  constructor(options = {}) {
    this.options = {
      bus: options.bus ?? 0,
      ...
      onDetection: options.onDetection ?? ((result) => {
        this.options.onMessage("---- fixed timing detection ----");
        ...
        this.options.onMessage(`fixedTimingUs:   ${result.baseUs}`);
        ...
        result.frames.forEach((frame, idx) => {
          this.options.onMessage(`  frame ${idx + 1}  score=${scoreFrame(frame)}  edges=${frame.length}`);
          this.options.onMessage(`    levels:      ${frame.map((edge) => edge.level).join(",")}`);
          this.options.onMessage(`    durations:   ${frame.map((edge) => edge.dtUs).join(",")}`);
  ...
  async getRssiRaw() {
    if (!this.radio) throw new Error("Fixed timing detector radio is not initialized");
  ...
  async start() {
    ...
    this.options.onMessage(
      `fixed timing detector started gdo0=${this.options.gdo0} threshold=${this.options.threshold} baseUs=${this.options.baseUs} lookbackMs=${this.options.lookbackMs} settleMs=${this.options.settleMs} rssiTolerance=${this.options.rssiTolerance ?? "off"}`
    );
      ...
      if (this.pendingTrigger && now - this.pendingTrigger.triggerTimeMs >= this.options.settleMs) {
        ...
        if (relevant.length >= this.options.minFrameEdges) {
          ...
          const frames = splitBySilence(quantized, this.options.silenceUnits)
            ...
            .slice(0, this.options.maxFrames);
